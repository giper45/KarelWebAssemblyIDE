...
// In public/shared.js
async function getKarelHeader() {
  const res = await fetch('/karel/karel.h');
...
const API = (function () {
  ...
  class App {
    constructor(module, memfs, name, ...args) {
      ...
      this.nextHandle = 0;
      this.animationId;
      this.timers = new Map(); // Aggiungi per gestire i timer
      this.nextTimerId = 0;    // ID progressivo per i timer
    ...
    canvas_test() { console.log("IN CANVAS TEST"); }
    canvas_sleep(seconds) {
      // Implementazione sincrona con busy waiting
      const ms = seconds * 1000;
      return new Promise(resolve => setTimeout(resolve, ms));

      // const start = performance.now();
      // const duration = seconds * 1000; // Converti in millisecondi
    ...
    // Avvia un timer e restituisce un ID
    canvas_start_timer() {
    ...
    // Verifica se sono passati tot secondi da quando è stato avviato il timer
    canvas_check_timer(timerId, seconds) {
      const startTime = this.timers.get(timerId);
      if (startTime === undefined) {
        console.warn(`Timer ${timerId} not found`);
        return 1; // Restituisce true se timer non trovato (per sicurezza)
      }

      const elapsed = performance.now() - startTime;
      const targetMs = seconds * 1000;
      return elapsed >= targetMs ? 1 : 0; // Restituisce 1 se tempo scaduto, 0 altrimenti
    }
  ...
  class API {
    constructor(options) {
      ...
      this.originalHostWrite = options.hostWrite;
      this.hostWrite = this.hostWriteWithCapture.bind(this);
      this.isCompiling = false 
      this.compilationOutput = ''; // Output captured during compilation
    ...
    hostWriteWithCapture(text) {
      if (this.isCompiling) {
        this.compilationOutput += text;
      }
      return this.originalHostWrite(text);
    }
    ...
    // Metodo per verificare se ci sono warning deprecation di Karel
    checkForKarelDeprecationWarnings(output) {
      const karelDeprecationPatterns = [
        /warning:.*'karel'.*is deprecated.*Direct access to karel is discouraged/i,
        /warning:.*'karel'.*is deprecated.*Use karel_get_/i,
        /warning:.*deprecated.*karel/i
      ];

      for (const pattern of karelDeprecationPatterns) {
    ...
    async compile(options) {
      ...
      this.compilationOutput = ''; // Reset output for this compilation
      this.isCompiling = true; // Flag to indicate compilation is in progress

      console.log(`Compiling ${input}...`);
      try {
        // Aggiungi automaticamente karel.h se il codice lo include
        if (contents.includes('#include "karel.h"') || contents.includes('#include <karel.h>')) {
          const karelHeader = await getKarelHeader()
          this.memfs.addFile('karel.h', karelHeader);
        }
        ...
        const result = await this.run(clang, 'clang', '-cc1', '-emit-obj',
          ...this.clangCommonArgs, '-O2', '-o', obj, '-x',
          'c++', input);

        if (this.checkForKarelDeprecationWarnings(this.compilationOutput)) {
          const errorMsg = '\x1b[91mCompilation failed: Karel deprecation warning detected!\x1b[0m\n' +
            '\x1b[93mYou are using deprecated direct access to Karel struct.\x1b[0m\n' +
            '\x1b[96mPlease use the accessor functions instead:\x1b[0m\n' +
            '  • Use \x1b[92mkarel_get_x()\x1b[0m instead of \x1b[91mkarel.x\x1b[0m\n' +
            '  • Use \x1b[92mkarel_get_y()\x1b[0m instead of \x1b[91mkarel.y\x1b[0m\n' +
            '  • Use \x1b[92mkarel_get_direction()\x1b[0m instead of \x1b[91mkarel.direction\x1b[0m\n' +
            '  • Use \x1b[92mkarel_get_bag_beepers()\x1b[0m instead of \x1b[91mkarel.bag_beepers\x1b[0m\n\n';

          this.originalHostWrite(errorMsg);
          throw new Error('Karel deprecation warning: Direct access to karel struct is not allowed');
        }
        return result;
      }
      finally {
        this.isCompiling = false; // Reset compilation flag
      }
    ...
    async run(module, ...args) {
      ...
      this.currentApp = app; // Store reference to current app
